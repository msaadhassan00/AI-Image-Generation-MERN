...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch('https://api-dall-e.netlify.app//api/v1/post', {
          method: 'POST',
          headers: {
  ...
  const generateImage = async () => {
    if(form.prompt){
      try {
        ...
        const response = await fetch('https://api-dall-e.netlify.app//api/v1/dalle',{
          method: 'POST',
          headers:{
            'Content-Type': 'application/json',
          },
